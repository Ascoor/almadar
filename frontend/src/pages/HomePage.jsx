...
function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-hero overflow-hidden">
        ...
        <div className="relative z-10 max-w-6xl mx-auto">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            >
              ...
              >
                <Button onClick={() => setShowLogin(true)} className="btn-hero text-lg px-8 py-4">
                  تسجيل الدخول
                </Button>
                <Button
                  variant="outline"
      ...
      {/* Features */}
      <motion.section initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="py-20 px-4 bg-background">
        <div className="max-w-6xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
            <h2 className="text-4xl font-bold text-foreground mb-4">ميزات المنصة</h2>
            <p className="text-xl text-muted-foreground">أدوات متقدمة لإدارة قانونية فعالة ومنظمة</p>
          </motion.div>

          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              { icon: FileText, title: 'إدارة العقود', description: 'إدارة شاملة بمراحل ومواعيد دقيقة' },
              { icon: Scale, title: 'الاستشارات القانونية', description: 'تنظيم الاستشارات بقاعدة بيانات احترافية' },
              { icon: Shield, title: 'التحقيقات والقضايا', description: 'متابعة منظمة مع إشعارات ذكية' },
              { icon: Users, title: 'إدارة المستخدمين', description: 'أدوار وصلاحيات مرنة وآمنة' },
            ].map((feature, i) => (
              <motion.div
                key={feature.title}
                initial={{ opacity: 0, y: 30 }}
