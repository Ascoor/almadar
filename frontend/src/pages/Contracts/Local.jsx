...
export default function Local({ contracts = [], onEditContract, onSelectContract }) {
    ...
    const fileUrl = selected.attachment 
    ? `${import.meta.env.VITE_API_URL}/storage/${selected.attachment}` 
    : null;
  ...
  {contract.attachment ? (
    <a 
      href={`${import.meta.env.VITE_API_URL}/storage/${contract.attachment}`}
      target="_blank"
