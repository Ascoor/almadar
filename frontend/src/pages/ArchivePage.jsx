...
function getLabel(type, lang) {
  ...
  return label
    ? label[lang]
    : type || (lang === 'ar' ? 'غير معروف' : 'Unknown');
}
    ...
    >
      ...
      <div className="mt-3 flex items-center gap-2 text-[10px] text-[var(--muted-foreground)]">
        ...
        <span className="ms-auto inline-flex gap-1 opacity-0 transition group-hover:opacity-100">
          <IconButton
            icon={Eye}
            label={COPY.preview[lang]}
            onClick={() => onPreview(file)}
          />
          <IconButton
            icon={FilePenLine}
            label={COPY.edit[lang]}
            onClick={() => onPreview(file, 'edit')}
          />
          <IconButton
            icon={PanelRightOpen}
            label={COPY.activeFile[lang]}
            onClick={() => onPreview(file)}
          />
        </span>
      </div>
            ...
            <div className="flex flex-wrap items-start gap-3 sm:items-center">
              ...
              <div className="ms-auto flex items-center gap-2 text-xs text-[var(--muted-foreground)] sm:text-sm">
                <span className="rounded-full bg-[var(--muted)] px-2 py-1">
                  {getLabel(activeFile.model_type, lang)}
                </span>
                {date && (
                  <span>
                    {new Date(date).toLocaleDateString(
          ...
          <div className="min-h-[280px] flex-1 overflow-auto bg-[var(--card)]">
            ...
            >
              ...
              fileUrlForPdf ? (
                ...
                <DocumentEditor
                  initialContent={buildEditorContent(activeFile, lang)}
                />
              )}
            </Suspense>
          </div>
...
export default function ArchivePage() {
  ...
  const fetchFiles = useCallback(
    async (withToast = false) => {
      try {
        ...
        toast.error(
          lang === 'ar' ? '❌ فشل تحميل الملفات' : '❌ Failed to load files',
        );
      } finally {
        setLoading(false);
        setRefreshing(false);
  ...
  const handleCardPreview = useCallback(
    (file, mode = 'auto') => {
      setActiveFile(file);
      setViewerMode(
        mode === 'auto' ? (isPdfFile(file) ? 'preview' : 'edit') : mode,
      );
      if (isMobile) {
  ...
  return (
    ...
    >
      ...
      <main className="container mx-auto max-w-7xl px-4 pb-10 pt-6 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="grid grid-cols-2 gap-2 sm:w-auto sm:grid-cols-3 sm:gap-3">
              ...
              <div className="hidden items-center justify-end gap-3 rounded-xl bg-[var(--background)] px-3 py-2 text-xs text-[var(--muted-foreground)] shadow-[var(--shadow-xs)] sm:flex">
                ...
                <span>
                  {COPY.statsVisible[lang]}: {filteredCount} /{' '}
                  {COPY.statsTotal[lang]}: {totalCount}
                </span>
              </div>
            </div>
          </div>
        ...
        <section
          ref={previewRef}
          className="hidden lg:block lg:sticky lg:top-[96px]"
        >
          <ViewerShell
            lang={lang}
        ...
        {activeFile && (
          <div className="relative rounded-t-3xl border border-[var(--border)] bg-[var(--background)] p-3 shadow-[var(--shadow-lg)]">
            <div className="mb-3 flex items-center justify-center">
              <span
                className="h-1.5 w-12 rounded-full bg-[var(--muted)]"
                aria-hidden="true"
              />
            </div>
            <ViewerShell
