...
export default function Sidebar({ isOpen, onToggle, onLinkClick }) {
  ...
  // عند النقر على رابط، نؤخّر الغلق حتى تنجز الملاحة
  const handleLinkClick = () => {
    if (!isLargeScreen && onLinkClick) {
      setTimeout(onLinkClick, 50);
    }
  };

  const handleSectionClick = (id) => {
    // لو الشاشة صغيرة والشريط مغلق، نفتحه أولًا
  ...
  return (
    <aside
      dir="rtl"
      className={`
        fixed right-0 z-20 top-0 h-full text-gray-100 bg-white dark:bg-black
        ...
        ${isLargeScreen
          ...
          : isOpen ? 'mt-16 translate-x-0 w-full' : 'translate-x-full'}
      `}
    >
