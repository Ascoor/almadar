...
export default function DropdownNotifications() {
  ...
  return (
    <div className="relative" dir="rtl" ref={ref}>
      ...
      {open && (
        <div className="absolute left-0 z-50 w-72 sm:w-80 mt-2 bg-white dark:bg-gray-800 rounded-xl shadow-lg ring-1 ring-black ring-opacity-5">
          ...
          <ul className="max-h-80 overflow-y-auto divide-y divide-gray-100 dark:divide-gray-700">
            {notifications.length === 0 ? (
              ...
              >
                ...
                <div className="text-xs text-gray-400 mt-1">{new Intl.DateTimeFormat('ar-EG',{dateStyle:'short',timeStyle:'short'}).format(new Date(n.created_at))}</div>
              </li>
            ))}
          </ul>
        </div>
