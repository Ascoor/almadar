...
export function DetailsShell({
  ...
  return (
    <div
      className="relative mt-4 rounded-[1.75rem] border border-border bg-card/80 backdrop-blur shadow-[var(--shadow-lg)] overflow-hidden"
    >
      ...
      <div className="p-6 md:p-10 space-y-8">
        <div className="flex items-center gap-3">
          <div className="grid place-items-center w-11 h-11 rounded-2xl border border-border bg-muted/30 shadow-[var(--shadow-sm)]" aria-hidden="true">
  ...
  return (
    <div className={clsx('flex items-start gap-2 text-fg', className)}>
      ...
      <div>
        ...
        <div className={clsx('font-semibold', !value && 'text-muted-foreground')}>{value || '—'}</div>
      </div>
    </div>
  );
}
...
export function SectionCard({ title, icon: Icon, children, hint, className }) {
  return (
    <div className={clsx('rounded-2xl border border-border bg-card/60 p-5 shadow-[var(--shadow-sm)] space-y-3', className)}>
      {(title || hint) && (
